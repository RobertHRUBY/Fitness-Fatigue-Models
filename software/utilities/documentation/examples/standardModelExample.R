...
# Set up constraints and fit the model ~ NOTE c(p*, kg, Tg, kh, Th)
  boxConstraints <- data.frame("lower" = c(5.0, 0.001, 1, 0.001, 1),
                               "upper" = c(6.0, 1, 50, 1, 50))
  boxConstraintsInitial <- data.frame("lower" = c(5.0, 0.001, 1, 0.001, 1, 0.1, 0.1),
                                      "upper" = c(6.0, 1, 50, 1, 50, 40, 40))
  
# Fit via first-order approach with second-order approximation
  startAt <- c(5.1, 0.1, 40, 0.2, 4) # Starting values c(p*, kg, Tg, kh, Th)
  startAt2 <- c(5.1, 0.1, 40, 0.2, 4, 5, 10) # Starting values c(p*, kg, Tg, kh, Th, qg, qh)
  
  # With no initial component
  fittedModel_bfgs <- standardModel(inputData = mockData,
  ...
  fittedModel_bfgs_initial <- standardModel(inputData = mockData,
...
# Fit via genetic algorithm and no initial component
  fittedModel_genetic <- standardModel(inputData = mockData,
                                       constraints = boxConstraints,
                                       method = "ga",
                                       doTrace = TRUE)
