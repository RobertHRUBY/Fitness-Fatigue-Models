...
vdrPredict <- function(vdrPars, loads, initialPars = c(0,0)){
  ...
  # If initial parameters q_g and q_h are supplied (otherwise evaluates to 0)
  initialFitness <- initialPars[1] * exp(-(1:T) / vdrPars[3])
  initialFatigue <- initialPars[2] * exp(-(1:T) / vdrPars[5])
  
  # Compute modeled fitness
  fitness <- vdrPars[2] * 
    base::sapply(1:T, function(i) convolveTraining(loads$load[1:i], vdrPars[3]))
  
  # For each i=1:T, compute k_h_2(i) = sum_(j=0)^(i){w(j)e^(-(i-j)/tau_h_2)} 
  # s.t. k_h_2(t) = sum(k_h_2(i)) for i = 1,2,...,(t-1)
  kh2 <- base::sapply(1:T, function(i) kh2Compute(loads$load[1:i], vdrPars[6]))
  
  # Compute modeled fatigue
  fatigue <- vdrPars[4] *
    base::sapply(1:T, function(i) convolveTraining(kh2[1:i], vdrPars[5]))
