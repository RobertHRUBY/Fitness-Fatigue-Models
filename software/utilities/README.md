...
<img src="https://latex.codecogs.com/svg.latex?\hat{p}(t)&space;=&space;p^*&space;&plus;&space;k_g&space;\sum_{i=1}^{t-1}\omega(i)(e^{\frac{-(t-i)}{\tau_g}})-k_h&space;\sum_{i=1}^{t-1}\omega(i)(e^{\frac{-(t-i)}{\tau_h}})" title="\hat{p}(t) = p^* + k_g \sum_{i=1}^{t-1}\omega(i)(e^{\frac{-(t-i)}{\tau_g}})-k_h \sum_{i=1}^{t-1}\omega(i)(e^{\frac{-(t-i)}{\tau_h}})" />

As a user, you have two options for importing the function into R. You can [download](standardModel.R) the file directly, and source or copy/paste it into your environment. Arguably though, the more convenient method is to make use of the package `devtools` that allows you to source the function directly into R from github as described further up in this README. This involves finding the [raw url](https://help.data.world/hc/en-us/articles/115006300048-GitHub-how-to-find-the-sharable-download-URL-for-files-on-GitHub) of the file. Once we have this, we run the following code:

    library(devtools)
    source_url(https://raw.githubusercontent.com/bsh2/Fitness-Fatigue-Models/main/software/utilities/standardModel.R)
    
You will notice the function `standardModel()` has appeared in your R global environment. This is the function we will use shortly.
