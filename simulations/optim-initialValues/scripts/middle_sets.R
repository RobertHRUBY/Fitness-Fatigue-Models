...
set.seed(101)
par_start_k1 <- rtruncnorm(n = n_starting_sets, 
                    a = round((upper_bound[1]-lower_bound[1])/2,0)-1, 
                    b = round((upper_bound[1]-lower_bound[1])/2,0)+1, 
                    mean = round((upper_bound[1]-lower_bound[1])/2,0), 
                    sd = 0.1)

set.seed(202)
par_start_T1 <- rtruncnorm(n = n_starting_sets, 
                    a = round((upper_bound[2]-lower_bound[2])/2,0)-2, 
                    b = round((upper_bound[2]-lower_bound[2])/2,0)+2, 
                    mean = round((upper_bound[2]-lower_bound[2])/2,0), 
                    sd = 0.5)

set.seed(303)
par_start_k2 <-rtruncnorm(n = n_starting_sets, 
                    a = round((upper_bound[3]-lower_bound[3])/2,0)-1, 
                    b = round((upper_bound[3]-lower_bound[3])/2,0)+1, 
                    mean = round((upper_bound[3]-lower_bound[3])/2,0), 
                    sd = 0.1)

set.seed(404)
par_start_T2 <-rtruncnorm(n = n_starting_sets, 
                    a = round((upper_bound[4]-lower_bound[4])/2,0)-2, 
                    b = round((upper_bound[4]-lower_bound[4])/2,0)+2, 
                    mean = round((upper_bound[4]-lower_bound[4])/2,0), 
                    sd = 0.5)
